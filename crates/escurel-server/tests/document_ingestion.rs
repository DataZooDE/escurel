...
#[cfg(feature = "kreuzberg")]
#[tokio::test]
async fn ingest_pptx_deck_end_to_end_materialises_searchable_instance() {
    // The exact analyst workflow: declare a `deck` document skill, upload a
    // real .pptx, and have escurel extract (kreuzberg) + chunk + materialise a
    // searchable, read-only instance whose original bytes are retained — the
    // deck then lives in the same referent space as every other page.
    let s = setup().await;
    s.indexer
        .update_page("markdown/skills/deck.md", DECK_SKILL)
        .await
        .unwrap();
    let token = s.process.mint_token(TENANT, Role::Agent);

    // A real PowerPoint deck (generated by python-pptx) with distinctive text.
    let pptx = include_bytes!("fixtures/deck.pptx");
    let blob = s
        .store
        .put_inbox_blob(TENANT, Bytes::from_static(pptx), None)
        .await
        .unwrap();

    // /ingest routes by MIME → the `deck` skill, extracts, chunks, materialises.
    let resp = post_ingest(&s.process, &token, blob.as_str(), PPTX_MIME).await;
    assert_eq!(
        resp["handler_skill"], "deck",
        "route the pptx by MIME: {resp}"
    );
    assert_eq!(
        resp["status"], "materialised",
        "kreuzberg PPTX extract: {resp}"
    );
    assert!(
        resp["chunk_count"].as_u64().unwrap() >= 1,
        "deck must yield chunks: {resp}"
    );
    let page_id = resp["page_id"].as_str().unwrap().to_owned();

    // The materialised instance is a read-only document overlay: it records the
    // kreuzberg extract engine and exposes bounded chunks via expand.
    let ex = call(&s.process, &token, "expand", json!({ "page_id": page_id })).await;
    let page = &ex["result"]["structuredContent"];
    assert_eq!(page["frontmatter"]["backend_ref"]["kind"], "document");
    assert_eq!(page["frontmatter"]["backend_ref"]["status"], "ok");
    assert!(
        page["frontmatter"]["backend_ref"]["extract_engine"]
            .as_str()
            .is_some_and(|e| e.contains("kreuzberg")),
        "PPTX must record the kreuzberg engine: {page}"
    );
    assert!(
        !page["blocks"].as_array().unwrap().is_empty(),
        "deck chunks indexed as blocks: {page}"
    );

    // The deck's own words are searchable — it's a first-class page now.
    s.indexer.refresh_fts().await.unwrap();
    let hits = call(
        &s.process,
        &token,
    ...
    let ids: Vec<String> = hits["result"]["structuredContent"]["hits"]
        ...
        .collect();
    assert!(
        ids.iter().any(|p| p == &page_id),
        "search must surface the ingested deck: {ids:?}"
    );

    // The original .pptx bytes are retained verbatim (faithful client preview).
    let r = call(
        &s.process,
        &token,
        "fetch_blob",

//! A ducklake reader's chat tool gate, LIVE (DuckLake PR 8, Phase B): a
//! REAL Postgres testcontainer backs both the DuckLake catalog AND the
//! shared chat table (`chat_pg.escurel_chat_messages`) `EscurelConfig::
//! build` attaches at boot when the catalog is Postgres-style.
//!
//! Before this PR, `append_message`/`list_messages` were on the reader's
//! static `UNSUPPORTED_ON_REPLICA_TOOLS` list unconditionally — this test
//! proves that gate is now dynamic: a reader booted against a Postgres
//! catalog serves them instead of rejecting them.
//!
//! Opt-in: gated behind the `live-ducklake` feature (needs Docker),
//! mirroring `escurel-index`'s own live tests. Run with `cargo test -p
//! escurel-server --features live-ducklake --test reader_role_chat`.
...
#[tokio::test]
async fn reader_no_longer_rejects_chat_tools_when_ducklake_configured() {
    ...
    let append_resp = call(
        &base,
        "append_message",
        serde_json::json!({
            "chat_group_id": "agent:dev-user",
            "role": "user",
            "content": "hi from a reader",
            "embed": false,
        }),
    )
    .await;
    assert!(
        append_resp.get("error").is_none(),
        "append_message on a Postgres-catalog reader must succeed, not \
         unsupported_on_replica: {append_resp}"
    );

    let list_resp = call(
        &base,
        "list_messages",
        serde_json::json!({ "chat_group_id": "agent:dev-user" }),
    )
    .await;
    assert!(
        list_resp.get("error").is_none(),
        "list_messages on a Postgres-catalog reader must succeed: {list_resp}"
    );
    let messages = &list_resp["result"]["structuredContent"]["messages"];
    assert_eq!(
        messages.as_array().map(|a| a.len()),
        Some(1),
        "the reader must read back the message it just wrote: {list_resp}"
    );
    assert_eq!(messages[0]["content"], "hi from a reader");

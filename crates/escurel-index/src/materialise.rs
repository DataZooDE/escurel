...
pub(crate) struct BlockRow<'a> {
    ...
    /// Already formatted as a DuckDB vector literal.
    pub dense_vec_sql: String,
}

/// Replace every `blocks` row for `page_id`.
///
/// The dense vector is interpolated rather than bound: DuckDB's parameter
/// binding has no array type that round-trips to `FLOAT[N]`, so the literal is
/// built by `format_vector_literal` and cast here. It is derived from
/// embedder output, never from user text.

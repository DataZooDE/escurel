...
| `list_skills` | — | list of `{id, description, required_frontmatter, optional_frontmatter, is_event_typed, visibility, owner_field?, autonomy?, layer, shadows?}` | the Tier-1 catalogue, **scoped to the caller**; `layer` is `"overlay"` (default) or the `base@<pack>@v<N>` pin; a shadowing overlay is ONE entry carrying `shadows: base@<pack>@v<N>`; `autonomy` is the declared human-in-the-loop policy — see the note below; `summary`, `harness`, `actions`, `cascade` are the workbench's skill-contract keys, present only when declared (see references/01) Rows carry `params` (what one RUN takes) and `fields` (the typed shape of the INSTANCES — `{name, kind, required, values?, target_skill?, min?, max?, label?, description?, render?}`; `render` is a display hint, see references/01) and `blocks` (the declared instance-body layout, `[{anchor, title?, kind?}]`, verbatim); each is omitted entirely for a skill that declares none |

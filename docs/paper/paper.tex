...
\item Feasibility evidence for three properties of the arrangement, bounded
  by an explicit account of what the measurements do not reach
...
Table~\ref{tab:guarantees} states eighteen properties, each verified by an
integration test executing in the project's ordinary test command; none is
disabled. The register is engineering discipline rather than validation of
the model: it establishes that the implementation does what it says, not that
the arrangement is preferable to another. We deliberately do not tabulate a
comparison against vector stores here. Production retrieval systems are
routinely wrapped in schemas, access control, versioning and transaction
logs, so a column asserting they lack these properties would compare against
a configuration rather than against any named system.
...
The eighteenth property records a limit of the present implementation rather
than of the model: the specification describes per-tenant handles within one
process, these are not implemented, and the deployed isolation boundary is
therefore the process. The property was added after an isolation defect in
which authentication resolved a tenant from the presented token while the
data path used the served tenant, and the two were never compared. The
general observation is that a test suite can encode a defect as intended
behaviour---here a foreign token succeeding was read as evidence that quota
accounting was per-tenant---and that enumerating properties independently of
the tests is what surfaces such cases.
...
\subsection{Retrieval Mechanics (Not Evidence for the Position)}
This subsection reports implementation context and bears on none of the
claims above; a reader assessing the position may skip it. Over a public
benchmark
...
The behavioural question remains open. Establishing that typed memory helps
agents, as distinct from establishing that it is cheap to offer, requires
multi-type corpora with type-naming queries, held-out tasks and ablations. A
synthetic construction is feasible and is the next step we intend; its
weakness is that a partitioning chosen by the authors of the system under
test is contestable, which argues for publishing the generator and the tasks
separately from any system that consumes them.

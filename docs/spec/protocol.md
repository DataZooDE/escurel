...
| `import_pack` | `{tenant_id, manifest, tarball_b64, allow_vertical_mismatch?}` | `{pack, version, vertical, pages_imported, layer}` | import a signed pack as the tenant's pinned, **read-only base layer**: signature + `content_hash` verify fail-closed **before** unpacking (`pack_signature_invalid`); unsafe entry paths refuse (`pack_malformed`); pages land under the reserved `markdown/base/<pack>/` namespace stamped `layer: base@<id>@v<version>`; the pin is recorded in `pack_subscriptions` (a canonical input, like the credential registry). A version change on a subscribed pack refuses (`pack_version_pinned` — upgrades are an explicit future `rebase`); an unrelated vertical refuses (`vertical_mismatch`) unless `allow_vertical_mismatch`. Transport-neutral: an air-gapped tarball and a live pull are the same call. See ADR-0007 |

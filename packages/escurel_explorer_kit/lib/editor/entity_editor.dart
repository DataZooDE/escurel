...
import 'backend_pane.dart';
import 'page_form.dart';
import 'shadow_pane.dart';
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Container(
      ...
      child: async.when(
        ...
        data: (page) => page == null
            ? const _EmptyState()
            : _PageView(page: page, showLinks: showLinks),
      ),
    );
  }
}
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'No page open',
              style: text.titleMedium?.copyWith(color: kOnSurfaceVariant),
            ),
...
class _PageView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final text = Theme.of(context).textTheme;
    final title =
        (page.frontmatter['name'] as String?) ??
    ...
    final skills =
        ref.watch(skillsCatalogueProvider).asData?.value ??
        const <SkillSummary>[];
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            key: const ValueKey('entity_editor.title'),
          ),
          ...
          if (editing && skill != null)
            ...
            // External-backend section (sql_view projection / document
            // chunks + status) — renders nothing for native markdown.
            BackendPane(page: page),
            // Shadow-drift section for a shadowing overlay skill page
            // (REQ-LAYER-03) — renders nothing for non-shadowing pages.
            ShadowPane(page: page),
...
class _BodyBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [for (final line in lines) _renderLine(context, line, text)],
    );
  }
        ...
        child: Row(
          ...
          children: [
            Padding(
              ...
              child: Text(
                '•',
                style: text.bodyMedium?.copyWith(color: kOnSurfaceVariant),
              ),
            ),
...
class _InlineMarkdown extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final base = Theme.of(
      context,
    ).textTheme.bodyMedium?.copyWith(color: kOnSurface);
    ...
    return Text.rich(
      TextSpan(
        style: base,
        children: inlineMarkdownSpans(context, text, skillIds),
      ),
    );
  }
}
...
class _MdLink extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final child = Text.rich(
      TextSpan(
        style: linkStyle,
        children: inlineMarkdownSpans(context, label, skillIds),
      ),
    );
